...
export const authenticateToken = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!token) {
        res.status(401).json({
            ...
            steps: [
                '1. Create a user with the register endpoint',
                '2. Login with the login endpoint to get a token',
                '3. Use the token by adding it to the Authorize button in the swagger UI'
            ]
        });
        return;
    }
